...
class _ExpensePieChartWidgetState extends ConsumerState<ExpensePieChartWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final list = pieServices.getExpenseData(dateTime: DateTime.now());
    final totalAmount = pieServices.getExpenseAmount(DateTime.now());

    List<Widget> labels(int index) {
      if (list.isEmpty) {
        return [
          Text(
            'No expense this month'.hardcoded,
    ...
    return DashboardWidget(
      title: 'Monthly expense'.hardcoded,
      child: SizedBox(
        height: 160,
        child: Row(
