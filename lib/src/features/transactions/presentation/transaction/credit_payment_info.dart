                          ...
                          ? Column(
                              ...
                              children: [
                                _Header(
                                  ...
                                  h2: 'Carry: ${balanceToPay(context)} ${context.currentSettings.currency.code} ${interest! > 0 ? '+~ ${interestString(context)} ${context.currentSettings.currency.code} interest' : ''}',
                                ),
                                ...
                                widget.chosenDateTime == null ||
                                        widget.chosenDateTime!
                                            .isAtSameMomentAs(widget.statement!.dueDate)
                                    ? _Header(
                                        isSelectedDay: widget.chosenDateTime == null
                                            ? false
                                            : widget.chosenDateTime!
                ...
                child: Wrap(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Text(
                          '${interest! > 0 ? '~ ' : ''}${fullPaymentAmount(context) ?? ''} ${context.currentSettings.currency.code}',
                        ...
                        HelpButton(
                          text:
                              'For easier tracking, you can only pay for transactions happens before selected day'
                                  .hardcoded,
                          size: 18,
                        ),
                      ],
          ...
          child: Row(
            children: [
                      ...
                      statement.checkpoint!.unpaidOfInstallments != 0
                          ? Text(
                              'Inst. left: ${CalService.formatCurrency(context, statement.checkpoint!.unpaidOfInstallments)} ${context.currentSettings.currency.code}'
                                  .hardcoded,
                              style: kHeader3TextStyle.copyWith(
                                  fontSize: 10, color: context.appTheme.backgroundNegative),
                              maxLines: 1,
                              overflow: TextOverflow.fade,
                            )
                          : Gap.noGap,
              ...
              Text(
                ...
                style:
                    kHeader2TextStyle.copyWith(fontSize: 13, color: context.appTheme.backgroundNegative),
              ),
              Text(
                ' ${context.currentSettings.currency.code}'.hardcoded,
                style:
                    kHeader4TextStyle.copyWith(fontSize: 13, color: context.appTheme.backgroundNegative),
              ),
            ],
          ),
        ),
...
extension _ListGetters on State<_List> {
  ...
  List<BaseCreditTransaction> get txnsInBillingCycleBeforePreviousDueDate {
    ...
    final list = widget.statement!
        .transactionsInBillingCycleBefore(widget.chosenDateTime ?? widget.statement!.dueDate);
  ...
  List<BaseCreditTransaction> get txnsInGracePeriod {
    ...
    return widget.statement!
        .transactionsInGracePeriodBefore(widget.chosenDateTime ?? widget.statement!.dueDate);
  }
  ...
  double? get interest {
    ...
    return widget.statement!.previousStatement.interest;
  }
  ...
  String? fullPaymentAmount(BuildContext context) {
    ...
    return CalService.formatCurrency(
      context,
      widget.statement!.getBalanceAmountAt(widget.chosenDateTime ?? widget.statement!.dueDate),
    );
  }
}
